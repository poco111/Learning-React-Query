...
export function Posts() {
  ...
  // replace with useQuery
  const { data } = useQuery({ queryKey: ['post'], queryFn: fetchPosts });
...
- v3와 달리 v5에서는 useQuery의 매개변수로 객체형태로 인수를 전달해야 한다
  - `const { data } = useQuery("post", fetchPosts);` -> v3 작성
  - `const { data } = useQuery({ queryKey: ['post'], queryFn: fetchPosts });` -> v5 작성
